...
func before_each():
	...
	# Create and add children so they are freed with the handler
...
func test_check_dirs_logic():
	_handler.check_dirs()
	var path = _handler.config.get_setting(_handler.config.SETTING_DOWNLOAD_PATH, _handler.config.DEFAULT_DOWNLOAD_PATH)
