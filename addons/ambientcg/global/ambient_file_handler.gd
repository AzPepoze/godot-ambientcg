...
func download_file_from_data(file_information: Dictionary, source_window: Window) -> void:
	...
	# await open_file_dialog_for_path(file_information.get("local_file_name", ""),
	# file_information.get("extension", ""))
	var path: String = "user://" + file_information.get("local_file_name", "")
	...
	await AmbientAPI.http_request_download(
		file_information.get("uri", ""), path, file_information.get("file_size", 0)
	)
...
func extract_and_save(
	window: Window,
	source_file: String,
	files_selected: Dictionary[String, CheckBox],
	options: Dictionary = {}
) -> void:
	...
	for file in files:
		if (
			files_selected.has(file)
			and (files_selected.get(file, CheckBox.new()) as CheckBox).button_pressed
		):
					...
					if options.get("use_custom_size", false):
						...
						if scaled_image and not scaled_image.is_empty():
							...
							if not new_size == Vector2(scaled_image.get_size()):
								scaled_image.resize(
									new_size.x, new_size.y, Image.INTERPOLATE_BILINEAR
								)
	...
	if options.get("enable_packing", false):
		...
		ResourceSaver.save(
			mat,
			(
				extraction_path.trim_suffix("/")
				+ "/%s_orm.material" % source_file.get_file().get_basename()
			)
		)
	else:
		...
		ResourceSaver.save(
			...
			(
				extraction_path.trim_suffix("/")
				+ "/%s_standard.material" % source_file.get_file().get_basename()
			)
		)
