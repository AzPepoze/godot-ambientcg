...
func api_info_to_option_button(
	button: OptionButton, json: Dictionary = AmbientAPI.api_information
) -> void:
	button.clear()

	var choices: Array = (
		get_parameter_from_key_and_type("type", "select", asset_list_query(json)).get("choices", [])
	)
...
func parse_search_query_data(json: Dictionary) -> Dictionary:
	...
	var id: String = (
		get_parameter_from_key_and_type(
			"type", "text", asset_list_query(AmbientAPI.api_information)
		)
		. get("id", "")
	)
	var payload_str = (
...
# in theory it could save the .tres file then auto populate the deps
# but it caused a missing dependency error when i tried it - cslr
