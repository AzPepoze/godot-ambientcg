...
func make_standard_material(files: PackedStringArray, options: Dictionary) -> StandardMaterial3D:
	...
	for file: String in files:
		...
		if file.containsn("displacement") or file.containsn("height"):
			# height maps dont support being used on triplanar materials.
			material.heightmap_enabled = not options.get("use_triplanar_uv", false)
			material.heightmap_texture = load(file)

	return material


func make_orm_material(_files: PackedStringArray, _options: Dictionary) -> ORMMaterial3D:
