...
func extract_all(source_file: String, target_path: String = "", options: Dictionary = {}) -> void:
	...
	if Engine.is_editor_hint():
		var ei = Engine.get_main_loop().root.get_node_or_null("EditorInterface")
		if ei and ei.has_method("get_resource_filesystem"):
			file_sys = ei.get_resource_filesystem()
	...
	if options.get("enable_packing", false):
		var mat = material_maker.make_orm_material(saved_files, options)
		ResourceSaver.save(mat, mat_save_path + ".tres")
	else:
		var mat = material_maker.make_standard_material(saved_files, options)
		ResourceSaver.save(mat, mat_save_path + ".tres")
