@tool
class_name AmbientDownloadWindow extends Window
...
func prompt_user_for_extraction(source_file: String) -> void:
	...
	if not found_tres.is_empty():
		ignoring_tres = not await root_ui.popup_accept(
			".tres File Found!", ".tres file found. Use %s instead?" % found_tres
		)
	...
	if ignoring_tres:
		...
		var directory := await AmbientFileHandler.open_directory_dialog_for_path(
...
func extract_button_pressed() -> void:
	...
	AmbientFileHandler.extract_and_save(
		self, extraction_source_file, extraction_file_links, options_dict
	)
