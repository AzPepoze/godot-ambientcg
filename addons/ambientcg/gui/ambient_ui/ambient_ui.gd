@tool
class_name AmbientUI extends Control
...
func search(search_query: String = "", use_next_query: bool = false):
	...
	# but it should assume its a shortlink and automatically find the download
	# https://github.com/VenitStudios/AmbientCG/issues/7
	...
	# avoid parsing if awaiting_search_finish is false,
	# can happen if a new search is started while waiting for a response
